...
class UsersAPI:
    ...
    def authorized(func):
        ...
        def wrapper(*args, **kwargs) -> web.Response:
            ...
            if not session:
                raise web.HTTPUnauthorized(text=json.dumps({
                    'status': 'error',
                    'message': 'Please sign in to use functionality.'
                }))
    ...
    def logout(session_id: str):
        ...
        db = DataBase()
        db_session = db.create_session()
        deleted = db_session.query(db.Session).filter_by(uuid=session_id).delete()
        db_session.commit()

        assert deleted, "The session must be deleted"
